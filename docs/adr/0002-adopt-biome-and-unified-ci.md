...
`typecheck + test + build + check:class-api + check:jsdoc`, but CI ran a *different*
...
- **The architectural boundary** ("no cross-feature `*.class.ts` imports") is
  enforced by a custom `scripts/check-boundaries.mjs`, **not** Biome
  ...
  the existing `check-class-api.mjs` / `check-jsdoc.mjs` family.
- **One gate, everywhere.** A single `verify` script =
  `check:ci + typecheck + test + build + check:class-api + check:jsdoc +
...
- CI and pre-push now enforce the *same* thing; the class-API/JSDoc/boundary checks
